    ...
    ProvideAppBarActions(
        list = listOf(
            AppBarActionItem(
                ...
                action = {
                    onAction(FavoriteUIAction.OnReportAction(context, fact.factBase))
                }
            ),
            AppBarActionItem(
                iconId = R.drawable.ic_round_favorite_24,
                color = MaterialTheme.colorScheme.primary,
                action = {
                    onAction(FavoriteUIAction.OnFavoriteItemAction(fact))
                }
            )
        )
    )
